...
export interface ISupplierLpoStorage {
  ...
  // Quantity fetching from delivery notes or invoices
  fetchQuantitiesFromDeliveryOrInvoice(lpoId: string): Promise<any[]>;
}
...
export interface IDeliveryStorage {
  ...
  // Invoiced items operations
  getInvoicedItemsForDelivery(deliveryId: string): Promise<any[]>;
}
...
export interface IInvoiceStorage {
  ...
  generateInvoiceFromDelivery(
    ...
    invoiceType?: string,
    userId?: string,
    selectedDeliveryItemIds?: string[]
  ): Promise<Invoice>;
