...
import { CarrierStorage } from './carrier-storage.js';
...
export class ModularStorage extends BaseStorage {
  ...
  private carrierStorage: CarrierStorage;
  ...
  constructor() {
    ...
    this.carrierStorage = new CarrierStorage();
  ...
  // Carrier operations - delegate to CarrierStorage
  async getCarriers() {
    return this.carrierStorage.getCarriers();
  }

  async getCarrier(id: string) {
    return this.carrierStorage.getCarrier(id);
  }

  async createCarrier(carrier: Parameters<CarrierStorage['createCarrier']>[0]) {
    return this.carrierStorage.createCarrier(carrier);
  }

  async updateCarrier(id: string, carrier: Parameters<CarrierStorage['updateCarrier']>[1]) {
    return this.carrierStorage.updateCarrier(id, carrier);
  }

  async deleteCarrier(id: string) {
    return this.carrierStorage.deleteCarrier(id);
  }

  async getCarrierDetails(id: string) {
    return this.carrierStorage.getCarrierDetails(id);
  }

  async getCarrierShipments(carrierId: string, page?: number, limit?: number) {
    return this.carrierStorage.getCarrierShipments(carrierId, page, limit);
  }

  async getCarrierPerformanceMetrics(carrierId: string) {
    return this.carrierStorage.getCarrierPerformanceMetrics(carrierId);
  }
  ...
  async checkInventoryItemExists(itemId: string): Promise<boolean> {
    const item = await this.inventoryStorage.getInventoryItem(itemId);
  ...
  // Alias for getGoodsReceiptHeaders - used by /api/receipts endpoint
  async getReceipts() {
    return this.goodsReceiptStorage.getGoodsReceiptHeaders?.() ?? [];
  }
  ...
  async getPurchaseInvoicesByLpoId(lpoId: string) {
    return purchaseInvoiceStorage.getPurchaseInvoicesByLpoId(lpoId);
  }

  async getPurchaseInvoicesBySupplierQuoteId(supplierQuoteId: string) {
    return purchaseInvoiceStorage.getPurchaseInvoicesBySupplierQuoteId(supplierQuoteId);
  }
