  ...
  static async getItems(quoteId: string) {
    try {
      ...
      const mappedItems = items.map(item => ({
        ...
        itemDescription: item.itemDescription, // Fixed: frontend expects itemDescription
        quantity: item.quantity, // Already an integer
        unitPrice: item.unitPrice,
        lineTotal: item.lineTotal, // Changed from totalPrice to lineTotal
        unitOfMeasure: item.unitOfMeasure,
        specification: item.specification, // Fixed: frontend expects specification
        brand: item.brand,
        model: item.model,
        leadTime: item.leadTime,
        warranty: item.warranty,
        notes: item.notes,
        // Include discount and VAT fields
    ...
    try {
      ...
      if (data.items && Array.isArray(data.items)) {
        for (let i = 0; i < data.items.length; i++) {
          ...
          const lineTotal = isNaN(totalPrice) ? 0 : totalPrice;
          
          const itemQuery = `
            INSERT INTO supplier_quote_items (
              id, supplier_quote_id, line_number, item_description, 
              quantity, unit_of_measure, unit_price, line_total, specification, 
          ...
          await pool.query(itemQuery, [
            ...
            unitPrice.toString(),
            lineTotal.toString(), // Ensure this is never null
            item.specification || null,
            item.brand || null,
            item.model || null,
            item.warranty || null,
            item.leadTime || null,
            (item.discountPercent || 0).toString(),
            (item.discountAmount || 0).toString(),
            (item.vatPercent || 0).toString(),
            (item.vatAmount || 0).toString()
          ]);
        }
      }
