...
export function registerQuotationRoutes(app: Express) {
  ...
  app.post("/api/quotations", async (req, res) => {
    try {
      ...
      const quotation = await storage.createQuotation(quotationData, userId);
  ...
  app.put("/api/quotation-items/:id", async (req, res) => {
    try {
      ...
      ["quantity","costPrice","markup","unitPrice","discountPercentage","discountAmount","vatPercent","vatAmount","lineTotal"].forEach(k => { if (partial[k] !== undefined) partial[k] = Number(partial[k]); });
