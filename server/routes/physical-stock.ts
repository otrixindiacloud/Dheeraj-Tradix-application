  ...
  // ===== PHYSICAL STOCK COUNT ROUTES =====
    ...
    app.post("/api/physical-stock", async (req, res) => {
      try {
        ...
        if (!uuidRegex.test(itemId)) {
          return res.status(400).json({ message: "Invalid item ID format. Please select a valid item from the dropdown." });
        }

        // Verify item exists in inventory
        const itemExists = await storage.checkInventoryItemExists(itemId);
        if (!itemExists) {
          return res.status(400).json({ message: "Selected item does not exist in inventory." });
        }
    ...
    app.put("/api/physical-stock/:id", async (req, res) => {
      try {
        ...
        // Validate UUID format if itemId is being updated
        if (updateData.itemId) {
          const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
          if (!uuidRegex.test(updateData.itemId)) {
            return res.status(400).json({ message: "Invalid item ID format. Please select a valid item from the dropdown." });
          }

          // Verify item exists in inventory
          const itemExists = await storage.checkInventoryItemExists(updateData.itemId);
          if (!itemExists) {
            return res.status(400).json({ message: "Selected item does not exist in inventory." });
          }
        }
