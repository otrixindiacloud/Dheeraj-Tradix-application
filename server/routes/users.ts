...
router.post("/", async (req, res) => {
  try {
    const { username, email, firstName, lastName, role, profileImageUrl, password } = req.body;

    if (!username || !role) {
      return res.status(400).json({ error: "Username and role are required" });
    }

    // Create a name field from firstName and lastName, or use username as fallback
