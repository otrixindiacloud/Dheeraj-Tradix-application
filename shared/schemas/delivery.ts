...
export const deliveryItems = pgTable("delivery_items", {
  ...
  remainingQuantity: integer("remaining_quantity").notNull().default(0), // Remaining quantity to be delivered
...
// Delivery History - Track partial delivery history and remaining quantities
export const deliveryHistory = pgTable("delivery_history", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  deliveryId: uuid("delivery_id").references(() => deliveries.id, { onDelete: "cascade" }).notNull(),
  salesOrderItemId: uuid("sales_order_item_id").notNull(), // References sales_order_items.id
  itemId: uuid("item_id").references(() => items.id).notNull(),
  ...
  deliveryType: varchar("delivery_type", { length: 50 }).notNull(), // "Full", "Partial"
  deliveryStatus: varchar("delivery_status", { length: 50 }).notNull(), // "Pending", "Partial", "Complete", "Cancelled"
  deliveryDate: timestamp("delivery_date"),
...
export const insertDeliveryHistorySchema = createInsertSchema(deliveryHistory).omit({ 
  id: true, 
  createdAt: true 
});
...
export type InsertDeliveryHistory = typeof insertDeliveryHistorySchema;
