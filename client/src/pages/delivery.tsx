                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {viewingDeliveryItems.map((it: any, idx: number) => {
                            const qty = Number(it.deliveredQuantity ?? it.pickedQuantity ?? it.orderedQuantity ?? it.quantity ?? 0);
                            const unit = Number(it.unitPrice ?? 0);
                            const derivedTotal = qty * unit;
                            const total = Number.isFinite(Number(it.totalPrice)) && Number(it.totalPrice) > 0
                            ...
                            return (
                              <tr key={it.id || idx} className="border-b border-gray-100">
                                ...
                                <td className="px-3 py-2 text-right text-gray-700">{formatCurrency(unit)}</td>
