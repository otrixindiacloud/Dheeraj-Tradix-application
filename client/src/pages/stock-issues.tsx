...
export default function StockIssuesPage() {
  ...
  const handleAutomationData = (data: any) => {
    ...
    if (data.documentNumber) {
      // You can pre-fill issue number
    }
    if (data.documentDate) {
      // You can pre-fill issue date
    }
  ...
  const createIssueMutation = useMutation({
    mutationFn: async (data: StockIssueForm) => {
      ...
      const quantity = Number(data.quantityIssued);
  ...
  <span className="font-mono text-sm font-medium">{issue.issueNumber || "N/A"}</span>
    ...
    },
    {
      ...
      render: (_: any, issue: any) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <Minus className={`h-4 w-4 ${isNegative ? 'text-red-500' : 'text-gray-500'}`} />
      ...
      render: (_: any, issue: any) => (
        <div className="flex items-center gap-2">
          <Button
            ...
            size="sm"
            onClick={() => {
              setLocation(`/material-issue/${issue.id}`);
            }}
          >
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{selectedIssue.itemName || "N/A"}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-500">Quantity Issued</Label>
                    <p className={`text-sm font-medium ${(selectedIssue.quantity || 0) < 0 ? 'text-red-600' : 'text-gray-900'}`}>
                      {Math.abs(selectedIssue.quantity || 0)}
                    </p>
                  </div>
                  <div>
