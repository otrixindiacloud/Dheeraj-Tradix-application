...
function GoodsReceiptsTab() {
  ...
  // Central PDF generator for reuse (row + dialog) - Now uses server-side generation
  const generateGoodsReceiptPdf = async (r: any) => {
    try {
      const response = await fetch(`/api/goods-receipt-headers/${r.id}/pdf?mode=enhanced`);
      if (!response.ok) {
      ...
      a.href = url;
      a.download = `goods-receipt-${r.receiptNumber || r.id || 'document'}.pdf`;
            ...
            <DialogTitle className="flex items-center justify-between w-full">
              ...
              {selectedReceipt && (
                <Button
                  ...
                  onClick={() => generateGoodsReceiptPdf(selectedReceipt)}
