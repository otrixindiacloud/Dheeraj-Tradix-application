...
export default function MaterialRequestsPage() {
  ...
  const handleAddDepartment = () => {
    ...
    if (allDepartments.includes(trimmedDept)) {
      toast({
        title: "Error",
    ...
    form.setValue("department", trimmedDept);
    setNewDepartment("");
  ...
  // Combine predefined departments with unique departments from requests
  const allDepartments = Array.from(new Set([...departments, ...uniqueDepartments])).sort();
      ...
      render: (_: any, request: Requisition) => (
        <div className="flex items-center gap-2">
          <Button
            ...
            size="sm"
            onClick={() => {
              navigate(`/material-requests/${request.id}`);
            }}
          >
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <StatusChangeDropdown
                        currentStatus={selectedRequest.status || "Draft"}
                        onStatusChange={async (newStatus) => {
                          try {
                            await apiRequest("PUT", `/api/material-requests/${selectedRequest.id}`, { status: newStatus });
                            queryClient.invalidateQueries({ queryKey: ["material-requests"] });
                            toast({ title: "Success", description: `Status changed to ${newStatus}` });
                            // Update the selectedRequest state to reflect the change
