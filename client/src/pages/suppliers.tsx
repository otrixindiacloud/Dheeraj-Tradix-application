...
interface Supplier {
  ...
  name: string;
  supplierCode?: string;
  supplier_code?: string; // Backend may return snake_case
  ...
  address?: string;
  paymentTerms?: string;
  payment_terms?: string; // Backend may return snake_case
  isActive?: boolean;
...
export default function Suppliers() {
  ...
  const { data: suppliers = [], isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Debug: Log the first supplier to see field names
      if (data.length > 0) {
        console.log("Supplier data structure:", data[0]);
      }
      return data;
    },
  });
  ...
  const columns: Column<Supplier>[] = [
    ...
    },
    {
      ...
      render: (value: string, supplier: Supplier) => {
        ...
        const code = supplier.supplierCode || (supplier as any).supplier_code;
        return code || "-";
      },
    },
    {
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="Enter supplier code (optional)" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
