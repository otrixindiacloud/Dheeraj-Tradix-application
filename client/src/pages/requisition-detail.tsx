...
export default function RequisitionDetailPage() {
  ...
  const calculateVatAmount = (estimatedCost: string, vatPercentage: number): number => {
    const cost = parseFloat(estimatedCost) || 0;
    return (cost * vatPercentage) / 100;
  };

  // Discount calculation function
  const calculateDiscountAmount = (estimatedCost: string, discountPercentage: number): number => {
    const cost = parseFloat(estimatedCost) || 0;
  ...
  const [sendQuoteForm, setSendQuoteForm] = useState({
    ...
    notes: '',
    currency: 'BHD' as 'BHD' | 'AED' | 'SAR' | 'QAR' | 'KWD'
  });
  ...
  // Create supplier quote from requisition
  const createSupplierQuote = useMutation({
    mutationFn: async (formData: typeof sendQuoteForm) => {
      if (!requisitionId || items.length === 0) {
        throw new Error('No items available to create quote');
      }

      const quoteData = {
        supplierId: formData.supplierId,
        requisitionId: requisitionId, // Include requisitionId
        ...
        items: items.map(item => ({
          ...
          unitPrice: parseFloat(item.estimatedCost) || 0,
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.message || 'Failed to create supplier quote');
      }

      return res.json();
    },
    onSuccess: (data) => {
      toast({ title: 'Success', description: 'Supplier quote created successfully' });
      ...
      navigate(`/supplier-quotes/${data.id}`);
    },
    onError: (err: any) => toast({ title: 'Error', description: err?.message || 'Failed to create supplier quote', variant: 'destructive' })
  });
  ...
  const handleOpenSendQuoteDialog = () => {
        ...
        <div className="flex space-x-2">
          <Tooltip>
            <TooltipTrigger asChild>
              <Button 
                variant="outline"
                onClick={handleOpenSendQuoteDialog}
            ...
            <TooltipContent>
              {items.length === 0 ? 'Add items to requisition first' : 'Create supplier quote from requisition items'}
            </TooltipContent>
          </Tooltip>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  variant="outline"
                  onClick={() => {
                    // Bulk edit functionality could be added here
                    toast({
                      title: "Bulk Edit",
                      description: `Edit ${selectedItems.size} selected items`,
                    });
                  }}
                >
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() => {
                    // Bulk delete functionality could be added here
                    toast({
                      title: "Bulk Delete",
                      description: `Delete ${selectedItems.size} selected items`,
                    });
                  }}
                >
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50/80 backdrop-blur-sm">
                  <tr className="border-b border-slate-200">
                    <th className="text-left font-medium text-slate-600 px-3 py-2 w-[40px]">
                      <Checkbox
                    ...
                    <th className="text-left font-medium text-slate-600 px-3 py-2">VAT %</th>
                    <th className="text-left font-medium text-slate-600 px-3 py-2">VAT Amount</th>
                    <th className="text-left font-medium text-slate-600 px-3 py-2">Discount %</th>
                    <th className="text-left font-medium text-slate-600 px-3 py-2">Discount Amount</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {itemsLoading && (
                    <tr><td colSpan={11} className="px-4 py-6 text-center text-slate-500">Loading items...</td></tr>
                  )}
                  {!itemsLoading && items.length === 0 && (
                    <tr><td colSpan={11} className="px-4 py-6 text-center text-slate-500">No items added yet.</td></tr>
                  )}
                  {!itemsLoading && items.map((item: RequisitionItem, idx: number) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-3 py-2 align-top">{item.vatPercentage}%</td>
                        <td className="px-3 py-2 align-top">${Number(item.vatAmount || 0).toFixed(2)}</td>
                      <td className="px-3 py-2 align-top">{item.discountPercentage}%</td>
                        <td className="px-3 py-2 align-top">${Number(item.discountAmount || 0).toFixed(2)}</td>
                      <td className="px-3 py-2 align-top">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input type="number" step="0.01" value={newItem.estimatedCost} onChange={(e) => {
                      const cost = e.target.value;
                      const vatAmount = calculateVatAmount(cost, newItem.vatPercentage);
                      const discountAmount = calculateDiscountAmount(cost, newItem.discountPercentage);
                      setNewItem({ ...newItem, estimatedCost: cost, vatAmount, discountAmount });
                    }} />
                  </div>
                  <div>
                    <label className="text-sm font-medium">VAT %</label>
                    <Input type="number" step="0.01" value={newItem.vatPercentage} onChange={(e) => {
                      const vatPercentage = parseFloat(e.target.value) || 0;
                      const vatAmount = calculateVatAmount(newItem.estimatedCost, vatPercentage);
                      setNewItem({ ...newItem, vatPercentage, vatAmount });
                    }} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input type="number" step="0.01" value={newItem.vatAmount} readOnly className="bg-gray-50" />
                  </div>
                  <div>
                    <label className="text-sm font-medium">Discount %</label>
                    <Input type="number" step="0.01" value={newItem.discountPercentage} onChange={(e) => {
                      const discountPercentage = parseFloat(e.target.value) || 0;
                      const discountAmount = calculateDiscountAmount(newItem.estimatedCost, discountPercentage);
                      setNewItem({ ...newItem, discountPercentage, discountAmount });
                    }} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input type="number" step="0.01" value={newItem.discountAmount} readOnly className="bg-gray-50" />
                  </div>
                  <div>
            ...
            {editingItem && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium">Estimated Cost</label>
                    <Input type="number" step="0.01" value={editingItem.estimatedCost} onChange={(e) => {
                      const cost = e.target.value;
                      const vatAmount = calculateVatAmount(cost, editingItem.vatPercentage);
                      const discountAmount = calculateDiscountAmount(cost, editingItem.discountPercentage);
                      setEditingItem({ ...editingItem, estimatedCost: cost, vatAmount, discountAmount });
                    }} />
                  </div>
                  <div>
                    <label className="text-sm font-medium">VAT %</label>
                    <Input type="number" step="0.01" value={editingItem.vatPercentage} onChange={(e) => {
                      const vatPercentage = parseFloat(e.target.value) || 0;
                      const vatAmount = calculateVatAmount(editingItem.estimatedCost, vatPercentage);
                      setEditingItem({ ...editingItem, vatPercentage, vatAmount });
                    }} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium">VAT Amount</label>
                    <Input type="number" step="0.01" value={editingItem.vatAmount} readOnly className="bg-gray-50" />
                  </div>
                  <div>
                    <label className="text-sm font-medium">Discount %</label>
                    <Input type="number" step="0.01" value={editingItem.discountPercentage} onChange={(e) => {
                      const discountPercentage = parseFloat(e.target.value) || 0;
                      const discountAmount = calculateDiscountAmount(editingItem.estimatedCost, discountPercentage);
                      setEditingItem({ ...editingItem, discountPercentage, discountAmount });
                    }} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium">Discount Amount</label>
                    <Input type="number" step="0.01" value={editingItem.discountAmount} readOnly className="bg-gray-50" />
                  </div>
                  <div>
                    <label className="text-sm font-medium">Urgency</label>
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 p-3 rounded-lg">
              <h4 className="font-medium text-gray-900 mb-2 text-sm">Requisition Details</h4>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select 
                    value={sendQuoteForm.supplierId} 
                    onValueChange={(value) => setSendQuoteForm({ ...sendQuoteForm, supplierId: value })}
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select 
                    value={sendQuoteForm.priority} 
                    onValueChange={(value: any) => setSendQuoteForm({ ...sendQuoteForm, priority: value })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input 
                    type="date" 
                    value={sendQuoteForm.validUntil} 
                    onChange={(e) => setSendQuoteForm({ ...sendQuoteForm, validUntil: e.target.value })}
                ...
                <div>
                  ...
                  <Select 
                    value={sendQuoteForm.currency} 
                    onValueChange={(value: any) => setSendQuoteForm({ ...sendQuoteForm, currency: value })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input 
                    value={sendQuoteForm.paymentTerms} 
                    onChange={(e) => setSendQuoteForm({ ...sendQuoteForm, paymentTerms: e.target.value })}
                ...
                <div>
                  ...
                  <Input 
                    value={sendQuoteForm.deliveryTerms} 
                    onChange={(e) => setSendQuoteForm({ ...sendQuoteForm, deliveryTerms: e.target.value })}
              ...
              <div>
                ...
                <Textarea 
                  value={sendQuoteForm.notes} 
                  onChange={(e) => setSendQuoteForm({ ...sendQuoteForm, notes: e.target.value })}
            ...
            {items.length > 0 && (
              <div className="space-y-2">
                <h4 className="font-medium text-gray-900">Items to be quoted</h4>
                <div className="max-h-32 overflow-y-auto border rounded-lg">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="text-left px-2 py-1 font-medium text-xs">Description</th>
                        <th className="text-left px-2 py-1 font-medium text-xs">Qty</th>
                        <th className="text-left px-2 py-1 font-medium text-xs">Unit</th>
                        <th className="text-left px-2 py-1 font-medium text-xs">Est. Cost</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y">
                      {items.map((item, idx) => (
                        <tr key={idx}>
                          <td className="px-2 py-1 text-xs">{item.itemDescription}</td>
                          <td className="px-2 py-1 text-xs">{item.quantity}</td>
                          <td className="px-2 py-1 text-xs">{item.unitOfMeasure}</td>
                          <td className="px-2 py-1 text-xs">{item.estimatedCost}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button 
                onClick={() => createSupplierQuote.mutate(sendQuoteForm)} 
                disabled={createSupplierQuote.isPending || !sendQuoteForm.supplierId || !sendQuoteForm.validUntil}
                className="bg-blue-600 hover:bg-blue-700"
              >
                {createSupplierQuote.isPending ? 'Creating...' : 'Create & Send Quote'}
              </Button>
            </div>
          </div>
        </DialogContent>
