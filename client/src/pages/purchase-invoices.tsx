...
export default function PurchaseInvoicesPage() {
  ...
  const [statusInvoice, setStatusInvoice] = useState<PurchaseInvoice | null>(null);
  ...
  // Update purchase invoice status mutation
  const updatePurchaseInvoiceStatus = useMutation({
    mutationFn: async (data: { invoiceId: string; status: string }) => {
      const response = await fetch(`/api/purchase-invoices/${data.invoiceId}/status`, {
        method: 'PATCH',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to update purchase invoice status');
      }
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Purchase invoice status updated successfully",
      });
    ...
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to update purchase invoice status",
        variant: "destructive",
      });
    },
  });
      ...
      <Dialog open={showStatusDialog} onOpenChange={setShowStatusDialog}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <div>
              <Label>Invoice</Label>
              <div className="font-medium">{statusInvoice?.invoiceNumber}</div>
              <div className="text-sm text-gray-500">{statusInvoice?.supplierName}</div>
            </div>
            <div>
              <Label>Current Status</Label>
              <div className="font-medium">
                <StatusPill status={statusInvoice?.status.toLowerCase().replace(' ', '-') || ''} />
              </div>
            </div>
            <div>
              ...
              <Button 
                onClick={() => {
                  if (statusInvoice && newStatus) {
                    updatePurchaseInvoiceStatus.mutate({
                      invoiceId: statusInvoice.id,
                      status: newStatus
                    });
                  }
                }}
                disabled={updatePurchaseInvoiceStatus.isPending || !newStatus || newStatus === statusInvoice?.status}
              >
                {updatePurchaseInvoiceStatus.isPending ? "Updating..." : "Update Status"}
              </Button>
            </div>
          </div>
        </DialogContent>
