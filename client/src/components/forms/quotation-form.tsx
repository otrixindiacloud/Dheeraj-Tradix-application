...
export default function QuotationForm({ enquiryId: propEnquiryId, onSuccess, onCancel }: QuotationFormProps) {
  ...
  // Set default valid until when creating a fresh quotation (no enquiry)
  useEffect(() => {
    if (!enquiryId && !date) {
      const defaultValidUntil = new Date();
      defaultValidUntil.setDate(defaultValidUntil.getDate() + 30);
      setDate(defaultValidUntil);
      form.setValue("validUntil", defaultValidUntil.toISOString().split('T')[0]);
    }
  ...
  // Calculate totals (no discount on create page)
  useEffect(() => {
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {customerType ? (
                ...
                <div className="text-sm text-gray-600">Select a customer to view pricing summary.</div>
              )}
            </CardContent>
          </Card>
        </div>
